...
```mermaid
flowchart TD
    subgraph P1["Pass 1：不透明物体"]
        direction LR
        A1["不透明物体<br/>(spot cow)"] --> A2["Blinn-Phong<br/>光照计算"] --> A3["opaqueFBO<br/>(color + depth)"]
    end
    subgraph P2["Pass 2：透明物体收集"]
        B1["透明物体 ×3<br/>(RGB quads)"] --> B2["Fragment Shader：<br/>1. atomicCounterIncrement<br/>获取节点 ID<br/>2. imageAtomicExchange<br/>头插法入链表<br/>3. 写入 linkedListBuffer"]
        B2 --> B3["SSBO 链表<br/>（每像素一条）"]
        B2 --> B4["oitRenderFBO<br/>(color attachment)"]
    end
    subgraph P3["Pass 3：合成输出"]
        C1["Fullscreen Quad +<br/>compositeShader：<br/>1. 遍历当前像素的链表<br/>2. 去重<br/>3. 插入排序（从远到近）<br/>4. Back-to-Front 混合"]
        C1 --> C2["默认帧缓冲（屏幕）"]
    end
    P1 --> P2
    P2 --> P3
...
> - **Pass 1**：深度写入 ON，深度测试 LESS。
> - **Pass 2**：深度写入 OFF，深度测试 ON（共享 opaque 的 depth），作用是让被不透明物体遮挡的透明片段被正确丢弃；去重针对相邻三角形边界同一深度的片段。
> - **Pass 3**：执行前需 `glMemoryBarrier`，确保 SSBO/Image/Atomic 写入完成后才读取。
...
```mermaid
flowchart LR
    H["headPointers[300,200] = 5"] --> N5["Node[5]<br/>红色片段<br/>depth=0.3<br/>next=2"]
    N5 --> N2["Node[2]<br/>绿色片段<br/>depth=0.5<br/>next=0"]
    N2 --> N0["Node[0]<br/>蓝色片段<br/>depth=0.7<br/>next=0xFFFFFFFF"]
    N0 --> E["END"]
```
