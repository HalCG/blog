...
```mermaid
flowchart TD
    S1["1. initPeelBuffers<br/>清理与状态重置<br/>清空 fboAccum_ 与 fboPeel_<br/>深度清除值设为 0.0<br/>供第 1 层剥离无缝通过"]
    S1 --> A
    subgraph LOOP["2. peelAndBlend 逐层剥离循环（最大 kMaxDepthPeelLayers 层）"]
        A["A. 准备当前层渲染目标<br/>绑定 fboPeel_<br/>动态挂载 output 深度附件<br/>清颜色为 (0,0,0,0)<br/>深度清除值设为 1.0"]
        B["B. 渲染场景进行剥离<br/>depth_peeling_render.frag<br/>输入：上一层深度纹理<br/>GL_SAMPLES_PASSED<br/>统计通过的片元数量"]
        C["C. 混合当前层到累积缓冲<br/>绑定 fboAccum_<br/>depth_peeling_blend.frag<br/>禁用深度测试<br/>Front-to-Back 混合公式"]
        D{"D. 交换乒乓深度索引<br/>sampleCount == 0 ?"}
        A --> B
        B --> C
        C --> D
        D -- "否：继续下一层" --> A
    ...
    D -- "是：提前终止循环" --> S3
    S3["3. compositeToScreen<br/>背景合成与写屏<br/>读 fboAccum_ 累积颜色<br/>与背景色混合输出到窗口"]
```

> 步骤 B 的过滤规则：`gl_FragCoord.z <= frontDepth` 的片元直接 `discard`，只保留比上一层更深的片元；步骤 D 通过 `swap(inputDepthIndex, outputDepthIndex)` 交换乒乓深度索引。
