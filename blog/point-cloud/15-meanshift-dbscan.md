...
```mermaid
flowchart TD
    Start["初始化: 将所有点标记为未访问"] --> Loop["对每个未访问的点 x:"]
    Loop --> Shift["重复 MeanShift 漂移:"]
    Shift --> A["a) 找到以当前点为圆和半径 h 的所有点"]
    A --> B["b) 计算邻域点的几何质心"]
    B --> C["c) 将当前点移动到质心位置"]
    C --> D{"移动距离 < ε (收敛) ?"}
    D -->|否| Shift
    D -->|是| E["3. 将收敛到的众数标记为新簇<br/>4. 将路径上所有点分配到该簇"]
    E --> F{"还有未访问点 ?"}
    F -->|是| Loop
    F -->|否| G["合并过于接近的众数 (距离 < h)"]
```
...
```mermaid
flowchart TD
    Start["输入: 点云, ε, minPts"] --> Init["1. 将所有点标记为 unvisited"]
    Init --> Loop["2. 对每个 unvisited 点 p:"]
    Loop --> MarkP["a) 标记 p 为 visited"]
    MarkP --> NeighborsP["b) 找到 p 的 ε-邻域 N(p)"]
    NeighborsP --> CheckP{"|N(p)| < minPts ?"}
    
    CheckP -->|是| Noise["c) 标记 p 为 NOISE"]
    CheckP -->|否| NewCluster["d) 创建新簇 C, 将 p 归入 C<br/>将 N(p) 中的所有点加入种子集合 S"]
    
    NewCluster --> LoopS{"S 是否为空 ?"}
    LoopS -->|否| PopQ["取出 S 中的下一个点 q"]
    PopQ --> CheckQNoise{"q 是 NOISE ?"}
    
    CheckQNoise -->|是| Border["改判 q 为 C 的边界点"]
    CheckQNoise -->|否| CheckQUnvisited{"q 是 unvisited ?"}
    
    Border --> LoopS
    
    CheckQUnvisited -->|是| MarkQ["标记 q 为 visited<br/>找到 N(q)"]
    CheckQUnvisited -->|否| AssignC["如果 q 未归属任何簇, 归入 C"]
    
    MarkQ --> CheckQCore{"|N(q)| ≥ minPts ?"}
    CheckQCore -->|是| ExpandS["将 N(q) 加入 S (区域增长)"]
    CheckQCore -->|否| AssignC
    
    ExpandS --> AssignC
    AssignC --> LoopS
    
    LoopS -->|是| CheckNext{"还有 unvisited 点 ?"}
    Noise --> CheckNext
    
    CheckNext -->|是| Loop
    CheckNext -->|否| End["输出: 聚类标签"]
```
