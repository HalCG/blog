...
```mermaid
flowchart TD
    A["原始点云"] --> B["1. 预处理<br/>(下采样, 滤波, 法向量等)"]
    B --> C["2. 特征提取<br/>(FPFH/SHOT 描述子)"]
    C --> D["3. 粗配准 (RANSAC)<br/>• 基于特征匹配的对应点对<br/>• 几何一致性检验<br/>• 输出: 粗变换 T_coarse"]
    D --> E["4. 精配准 (ICP/NDT)<br/>• 以 T_coarse 为初始值<br/>• 输出: 精确变换 T_fine"]
```
...
```mermaid
flowchart TD
    Start["输入: 源点云 S, 目标点云 T, 匹配特征对"] --> Step1["1. 特征提取与初始匹配:<br/>计算 src_fpfh, tgt_fpfh<br/>建立相互最近邻 correspondences"]
    Step1 --> Loop["2. RANSAC 迭代 (for iter in range(max_iters)):"]
    
    Loop --> Sample["a) 随机采样 3 对对应点"]
    Sample --> VerifyGeo{"b) 验证几何一致性 (边长检验):<br/>源点三角形边长 ≈ 目标点三角形边长 ?"}
    
    VerifyGeo -->|否| Skip["跳过这组采样"]
    VerifyGeo -->|是| Estimate["c) 从 3 对点估计变换 (R, t)"]
    
    Estimate --> VerifyAll["d) 验证: 统计所有匹配对中符合该变换的内点数"]
    VerifyAll --> Update["e) 如果内点数更多，更新最佳变换 T_best"]
    
    Skip --> Next{"未达最大迭代数 ?"}
    Update --> Next
    
    Next -->|是| Loop
    Next -->|否| End["输出: 最佳变换矩阵 T_best"]
```
...
```mermaid
flowchart TD
    A["全局配准 (无需初始位姿)<br/>• RANSAC + 特征匹配<br/>• FPFH / SHOT / ISS 关键点 + 描述子<br/>• 相互最近邻匹配<br/>• 几何一致性检验 (边长比 / 三点约束)<br/>• 输出: 粗变换 (5-20 cm 精度)"] --> B["局部精配准 (需近似的初始位姿)<br/>• Point-to-Point ICP (SVD 解析解)<br/>• Point-to-Plane ICP (线性化地用法向量)<br/>• NDT (分段连续高斯 PDF)<br/>• 输出: 精细变换 (1-5 mm 精度)"]
```
