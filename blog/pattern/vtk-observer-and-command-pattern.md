...
```text
vtkObject::InvokeEvent          ← Observer（广播）
        ↓
vtkCommand::Execute             ← 回调对象（VTK 文档称 command）
        ↓
AbortFlag / GrabFocus / priority ← 责任链式分发
```

**对比记忆**：

```text
GoF Command:  用户点「移动」→ MoveCommand 入栈 → redo() / undo()
vtkCommand:   renderer 发 StartEvent → 已登记的 Execute() 被调用
```
...
```
vtkObjectBase          （引用计数、类型信息）
    └── vtkObject      （Modified、InvokeEvent、AddObserver）
            └── vtkAlgorithm / vtkRenderer / vtkInteractorStyle / ...

vtkObjectBase
    └── vtkCommand     （Execute 纯虚函数）
            ├── vtkCallbackCommand
            ├── vtkOldStyleCallbackCommand
            └── 各种模板包装（`vtkObject::AddObserver` 成员函数绑定等）
```
...
```
Phase 0: Passive Observer Loop   （被动观察者，最先执行）
Phase 1: Focus Loop              （若 GrabFocus 过，则只给焦点命令）
Phase 2: Remainder Loop          （普通观察者，按优先级/插入顺序）
```
...
OS 输入（鼠标/键盘）
    ...
    → vtkInteractorStyle::OnLeftButtonDown() 等虚函数
    → InvokeEvent(LeftButtonPressEvent)
    → 多个 vtkCommand::Execute()
    → 相机操作 / Widget / 自定义逻辑
    → 可能 Render()
```
...
```
InteractorStyle ──event──▶ SceneController ──event──▶ UI / 业务逻辑
```
...
```
用户平移模型 → TranslateCommand → Undo/Redo 栈
```
...
┌─────────────────────────────────────────────────────────────┐
│                    vtkRenderWindowInteractor                 │
│  捕获 OS 输入 → 调用 InteractorStyle 虚函数                  │
└───────────────────────────┬─────────────────────────────────┘
                            │ InvokeEvent(LeftButtonPressEvent)
                            ▼
┌─────────────────────────────────────────────────────────────┐
│              vtkInteractorStyle（vtkObject 子类）              │
│  Observer 链：Widget → InteractorStyle → 自定义 Command ...   │
│  可通过 AbortFlag 中断后续处理                                │
└───────────────────────────┬─────────────────────────────────┘
                            │ 更新相机 / 选择集 / Modified
                            ▼
┌─────────────────────────────────────────────────────────────┐
│                    vtkRenderer::Render()                     │
│  InvokeEvent(StartEvent) → 绘制 → InvokeEvent(EndEvent)      │
└───────────────────────────┬─────────────────────────────────┘
                            │
                            ▼
                   RenderWindow 呈现像素
...
贯穿全程的机制：
  vtkObject::AddObserver(event, vtkCommand*, priority)
  vtkCommand::Execute(caller, eventId, callData)
```
