...
```text
DIP（依赖抽象）──支撑──▶ OCP（扩展不改旧代码）
LSP（子类可信）──支撑──▶ OCP（多态扩展才安全）
SRP（职责单一）──减少──▶ 修改时的意外牵连
ISP（小接口）────减少──▶ 对实现的强迫
CRP（组合）──────落实──▶ DIP / OCP 而不搞深继承树
```
...
#### 深模块 vs 浅模块
```text
深模块：  调用方 ──简单 API──▶ [ 大量实现、隐藏细节 ]
浅模块：  调用方 ──仍要懂很多──▶ [ 薄薄一层 + 另一层也要懂 ]
```
...
│  元原则：控制复杂度（理解成本 + 修改成本）                  │
├─────────────────────────────────────────────────────────┤
│  结构层：SOLID + CRP + LoD + 高内聚低耦合                 │
│  变化层：DRY、正交、可逆、解耦、封装变化点、YAGNI ↔ OCP    │
│  模块层：深模块、信息隐藏、下拉复杂度、不同层不同抽象        │
│  人的层：命名、注释、一致性、显而易见、可测试、沟通          │
│  过程层：战略式编程、Design twice、重构、测试、够好即可      │
└─────────────────────────────────────────────────────────┘
```
