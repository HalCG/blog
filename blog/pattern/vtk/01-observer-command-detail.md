...
## 写在前面：名字像两种模式，机制其实是一套

很多人第一次读 VTK 文档时，会把「观察者模式」和「命令模式」当成两个独立话题。VTK 官方在 `vtkCommand.h` 的类注释里写：

> *vtkCommand is an implementation of the **observer/command** design pattern.*

**但按 GoF 教科书的标准，`vtkCommand` 并不是完整的「命令模式」**——它首先是 **Observer 的回调载体**（把监听逻辑封装成带 `Execute()` 的对象），再叠上 `priority`、`AbortFlag`、`GrabFocus` 等**事件链上的责任链语义**。VTK 文档里的 *command* 更多是历史命名和接口外形，不要和 `QUndoCommand`、应用层撤销栈混为一谈。

拆开理解更清楚：

- **观察者模式**解决：*谁订阅、谁广播、如何一对多通知*（`AddObserver` + `InvokeEvent`）；
- **`vtkCommand`** 解决：*通知到了以后执行什么回调、能否替换实现、能否截断后续 listener*；
- **责任链**（非 GoF Command）解决：*多个 listener 按优先级处理，某人 `AbortFlagOn()` 后后面不再执行*。

在 VTK 里，这三件事通过同一条 API 缝合在一起：
...
`AddObserver` 是观察者订阅；`vtkCommand` 是回调对象；`InvokeEvent` 是事件广播。  
**概念上分三层，API 上是一条链。**
...
### 1.2 `vtkCommand`：回调对象，而非完整 GoF Command

GoF **命令模式**的典型目标是：把「用户操作」封装成可替换、可排队、**可撤销**的对象：
...
classDiagram
  ...
  Invoker --> Command : 调用 execute() / undo()
  Command <|-- ConcreteCommand : "封装操作<br/>可替换实现<br/>可入栈、可重做"
```
...
但触发方式、生命周期和 GoF Command **根本不同**：

| | GoF Command（如 `QUndoCommand`） | `vtkCommand` |
|--|----------------------------------|--------------|
| 触发 | 用户 / 业务**主动 push** | Subject **`InvokeEvent` 广播** |
| 语义 | 可逆编辑操作 | 事件响应 |
| `undo()` | 有 | **无** |
| 命令队列 | 常见 | **无** |
| 截断后续处理 | 非典型 | **`AbortFlag`、priority** |

因此更准确的说法是：

```mermaid
flowchart TD
  A["vtkObject::InvokeEvent<br/>Observer（广播）"] --> B["vtkCommand::Execute<br/>回调对象（文档称 command）"]
  B --> C["AbortFlag / GrabFocus / priority<br/>责任链式分发"]
```
...
```mermaid
flowchart LR
  subgraph GOF["GoF Command"]
    A1["用户点「移动」"] --> A2["MoveCommand 入栈"]
    A2 --> A3["redo() / undo()"]
  end
  subgraph VTK["vtkCommand"]
    B1["renderer 发 StartEvent"] --> B2["已登记的 Execute() 被调用"]
  end
```

VTK 之所以仍叫 *observer/command*，是因为：① 用对象封装 `Execute` 便于与引用计数、C/C++ 回调兼容；② 可在运行期替换不同 `vtkCommand` 实现；③ 历史文档沿用了 command 一词。**这不等于 VTK 内核实现了完整的 GoF 命令模式。**

### 1.3 VTK 为什么要把 Observer 与回调对象焊在一起？

C++ 里没有 Java 式的 `interface Observer { void update(); }` 泛型回调体系，VTK 又需要：

1. 与引用计数体系 `vtkObjectBase::Register/UnRegister` 兼容；
2. 支持 C 风格静态回调、C++ 成员函数、Lambda 包装；
3. 支持**优先级**、**事件中断**、**焦点捕获**；
4. 在渲染与交互这种高频路径上保持足够轻量。

所以 VTK 没有单独做一个 `vtkObserver` 抽象类，而是让 **`vtkCommand` 承担 Observer 的回调职责**——主框架仍是 Observer，不是双模式并列。
...
classDiagram
  ...
  vtkCommand <|-- TemplateWrapper : "模板包装<br/>（成员函数绑定等）"
```
...
### 2.2 `vtkCommand`：Observer 的回调体（文档称 command）
...
```mermaid
flowchart TD
  P0["Phase 0: Passive Observer Loop<br/>（被动观察者，最先执行）"]
  P1["Phase 1: Focus Loop<br/>（若 GrabFocus 过，则只给焦点命令）"]
  P2["Phase 2: Remainder Loop<br/>（普通观察者，按优先级/插入顺序）"]
  P0 --> P1
  P1 --> P2
```
...
```mermaid
flowchart TD
  A["OS 输入（鼠标/键盘）"] --> B["vtkRenderWindowInteractor"]
  B --> C["vtkInteractorStyle::OnLeftButtonDown() 等虚函数"]
  C --> D["InvokeEvent(LeftButtonPressEvent)"]
...
```mermaid
flowchart LR
  A["InteractorStyle"] -- "event" --> B["SceneController"]
  B -- "event" --> C["UI / 业务逻辑"]
```

一层层都是 **Observer + 回调对象**，只是职责不同；这里的「command」指 `vtkCommand`，不是 GoF 可撤销命令。
...
1. 订阅事件前，先查 VTK 文档或源码中该 subject 的 `InvokeEvent` 调用点；
2. 不要假设所有 `LeftButtonPressEvent` 都带 `QMouseEvent*`——只有 Qt 集成层才这样；
3. 对 `callData` 做类型转换前，确认 subject 类型。

---

## 8. 与 Pipeline、GoF Command、应用层 Undo 的区分
...
### 8.2 `vtkCommand` 也不是 GoF Command

对照表：

| 能力 | GoF Command | `vtkCommand` |
|------|-------------|--------------|
| 行为对象化 | 有 | 有 |
| `Execute` 统一入口 | 有 | 有 |
| 由事件广播触发 | 否 | **是** |
...
**结论**：`vtkCommand` 是 Observer 事件系统里的 listener 对象；GoF 意义上的 Command 要在 UI / 业务层自己实现（见下节）。

### 8.3 应用层的 Undo/Redo Command 是另一套东西
...
| | `vtkCommand` | 应用层 Undo Command（GoF） |
|--|--------------|------------------------------|
| 层次 | VTK 内核**事件回调** | 业务逻辑 / 可撤销编辑 |
| 接口 | `Execute(caller, eventId, callData)` | `execute() / undo() / redo()` |
| 触发 | `InvokeEvent` 广播 | 用户操作主动入栈 |
| 目的 | 响应 VTK 对象生命周期与输入事件 | 记录用户可撤销操作 |
...
```mermaid
flowchart TD
  subgraph I["vtkRenderWindowInteractor"]
    A["捕获 OS 输入<br/>调用 InteractorStyle 虚函数"]
  end
  A -- InvokeEvent(LeftButtonPressEvent) --> B

  subgraph S["vtkInteractorStyle（vtkObject 子类）"]
    B["Observer 链<br/>Widget → InteractorStyle → 自定义 Command ...<br/>可通过 AbortFlag 中断后续处理"]
  end
  B -- "更新相机/选择集/Modified" --> C

  subgraph R["vtkRenderer::Render()"]
    C["InvokeEvent(StartEvent)<br/>绘制<br/>InvokeEvent(EndEvent)"]
  end
  C --> D["RenderWindow 呈现像素"]

  subgraph M["贯穿全程的机制"]
    E["vtkObject::AddObserver(event, vtkCommand*, priority)<br/>vtkCommand::Execute(caller, eventId, callData)"]
  end
```
...
| 概念 | 在 VTK 中的落点 | 本质问题 |
|------|----------------|----------|
| 观察者模式（主框架） | `vtkObject` + `AddObserver` + `InvokeEvent` | **谁通知谁？** |
| 回调对象（VTK 称 command） | `vtkCommand::Execute` | **通知后跑哪段逻辑？** |
| 责任链 | `priority` + `AbortFlag` + `GrabFocus` | **谁优先？何时停止传递？** |
| 渲染集成 | `vtkRenderer` 的 `Start/EndEvent` | **如何在绘制生命周期插入逻辑？** |
| 交互集成 | `vtkInteractorStyle` 输入事件 | **如何把鼠标键盘变成可订阅回调链？** |
| GoF 命令（不在 VTK 内核） | 应用层 `CommandManager` / `QUndoCommand` | **用户操作如何撤销？** |

VTK 实现的是一套 **以 `vtkObject` 为事件总线、以 `vtkCommand` 为回调执行体、带优先级与中断的观察者框架**。  
文档里的 *command* 是回调封装，**不是**完整的 GoF 命令模式；读懂 `vtkSubjectHelper::InvokeEvent` 的三段分发，就读懂了 VTK 事件系统的大半本质。

---

## 重点与注意（进阶速记）

> **重点**：主框架是 **Observer**；`vtkCommand` 是 listener 的对象化封装，VTK 文档称 *command*，**不等于 GoF 命令模式**。  
> **重点**：`InvokeEvent` 三阶段：Passive（只读）→ Focus（`GrabFocus`）→ Remainder；`AbortFlag` 为 true 则**整链截断**——这是**责任链**，不是 Command 的核心。  
> **重点**：`vtkCommand` ≠ `QUndoCommand` ≠ Pipeline ≠ 应用层 `TranslateCommand`；名字或场景相近，语义完全不同。  
