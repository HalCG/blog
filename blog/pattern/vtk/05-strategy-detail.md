...
```mermaid
flowchart TD
    A["OS 输入<br/>鼠标/键盘/触摸"] --> B["vtkRenderWindowInteractor<br/>Context（壳，路由事件）"]
    B -- "GetInteractorStyle()" --> C["vtkInteractorStyle<br/>Strategy：手势算法"]
    C -- "更新 Camera/Actor/Selection" --> D["vtkRenderer::Render()"]
    D --> E["vtkActor / vtkVolume"]
    E -- "GetMapper()" --> F["vtkMapper<br/>Strategy：如何把数据画出来"]
    F --> G["GPU / 帧缓冲"]
```
...
```mermaid
flowchart TD
    A["平台：鼠标左键按下"] --> B["vtkRenderWindowInteractor::LeftButtonPressEvent()"]
    B --> C["vtkInteractorStyle::OnLeftButtonDown()<br/>(虚函数，子类实现)"]
    C --> D["（Trackball）旋转相机 / （RubberBand）记录起点"]
...
```mermaid
flowchart TD
    Filter["vtkAlgorithm (Filter)"] --> DataSet["vtkDataSet"]
    DataSet --> Mapper["vtkMapper<br/>(Strategy：着色、图元类型、GPU 路径)"]
    Mapper --> Actor["vtkActor / vtkVolume"]
...
```mermaid
flowchart TD
    RenderWindow["vtkRenderWindow"] --> Renderer0["Renderer0"]
    RenderWindow --> Renderer1["Renderer1"]
    
    Renderer0 <--> Association0["Interactor0 + Style0"]
    Renderer1 <--> Association1["Interactor1 + Style1<br/>(或共享一个 Interactor)"]
```
...
```mermaid
flowchart TD
    Interactor["vtkRenderWindowInteractor<br/>SetInteractorStyle(vtkInteractorStyle*)"] --> Camera["TrackballCamera<br/>(浏览策略)"]
    Interactor --> Select["RubberBand2D<br/>(框选策略)"]
    Interactor --> Image["ImageStyle<br/>(窗宽窗位策略)"]
    
    Camera --> Render["vtkRenderer::Render()"]
    Select --> Render
    Image --> Render
    
    Render --> Actor["vtkActor<br/>SetMapper(...)"]
    Render --> Volume["vtkVolume<br/>SetMapper(...)"]
    
    Actor --> PolyMapper["PolyDataMapper<br/>(面片策略)"]
    Volume --> GPUMapper["GPUVolumeRayCastMapper<br/>(体绘制策略)"]
    
    PolyMapper --> Output["帧缓冲 / 显示"]
    GPUMapper --> Output
    
    clickSelected["点击选中：vtkAbstractPicker 策略 (Cell / Prop / Point ...)"] -.-> Interactor
```
