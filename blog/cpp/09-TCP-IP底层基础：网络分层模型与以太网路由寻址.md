...
```
     OSI 七层模型           TCP/IP 四层模型                   数据封装流向
 ┌──────────────────┐    ┌──────────────────┐         ┌────────────────────────┐
 │      应用层      │──┐ │                  │         │    用户应用层数据      │
 ├──────────────────┤  ├─│      应用层      │         ├────────────────────────┤
 │      表示层      │──┤ │ (HTTP, FTP, DNS) │         │ 应用首部 │ 用户数据    │
 ├──────────────────┤  │ │                  │         ├──────────┴─────────────┤
 │      会话层      │──┘ └────────┬─────────┘         │  TCP首部 │  应用数据   │
 ├──────────────────┤    ┌────────┴─────────┐         ├──────────┴─────────────┤
 │      传输层      │───►│  传输层 (TCP/UDP)│         │  IP首部  │  TCP数据段  │
 ├──────────────────┤    └────────┬─────────┘         ├──────────┴─────────────┤
 │      网络层      │───►│    网络层 (IP)   │         │ 帧首(MAC)│  IP数据包   │
 ├──────────────────┤    └────────┬─────────┘         └────────────────────────┘
 │    数据链路层    │──┐ ┌────────┴─────────┐                  ▼ 传输
 │      物理层      │──┴─│   网络接口/链路层│ ──► [ 01011100... 物理介质比特流 ]
 └──────────────────┘    └──────────────────┘
```
...
```
     主机 A (192.168.1.2)                          主机 B (192.168.1.3)
 ┌─────────────────────────┐                   ┌─────────────────────────┐
 │ 发送 ARP 广播询问：      │ ──► [局域网广播] ──►│ 发现询问的是自己的 IP， │
 │ "谁的 IP 是 192.168.1.3?│                   │ 回复单播 ARP 应答：     │
 │  请把 MAC 回复给 A"      │ ◄─── [单播回复] ───│ "我的 MAC 是 xx-xx-xx"  │
 └─────────────────────────┘                   └─────────────────────────┘
```
