...
```
  ...
  CLOSED                                                      LISTEN
     │                                                           │
     │  ─── 发送 SYN (seq=x, 建立连接请求) ─────────────────────►│  收到的 SYN 存入半连接队列
  SYN_SENT                                                    SYN_RCVD
     │                                                           │
     │  ◄── 回复 SYN + ACK (seq=y, ack=x+1, 确认并请求反向连接) ─│
     │                                                           │  连接移入全连接队列
  ESTABLISHED ─── 发送 ACK (ack=y+1, 确认建立) ─────────────────►│
     ▼                                                        ESTABLISHED
```
...
```
  ...
  ESTABLISHED                                                 ESTABLISHED
     │                                                           │
     │  ─── 发送 FIN (seq=u, 请求关闭我这一侧的输出) ────────────►│  关闭输入通道
  FIN_WAIT_1                                                  CLOSE_WAIT
     │                                                           │
     │  ◄── 回复 ACK (ack=u+1, 确认知悉) ────────────────────────│  继续发送未发完的数据...
  FIN_WAIT_2                                                     │
     │                                                           │  数据发完，准备完毕
     │  ◄── 发送 FIN (seq=v, 请求关闭我这一侧的输出) ────────────│  
  TIME_WAIT                                                   LAST_ACK
     │  ─── 发送 ACK (ack=v+1, 确认关闭) ───────────────────────►│
  等待 2MSL                                                   CLOSED
     ▼
...
```
                    客户端 SYN 握手包
                          ...
                          ▼
                ┌──────────────────┐
                │  半连接队列       │ <-- 存放处于 SYN_RCVD 状态的连接
                │  (SYN Queue)     │
                └─────────┬────────┘
                          │ (第三次握手完成)
                          ▼
                ┌──────────────────┐
                │  全连接队列       │ <-- 存放处于 ESTABLISHED 状态、等待 accept() 的连接
                │  (Accept Queue)  │
                └─────────┬────────┘
                          │ (调用 accept())
                          ▼
...
```
  已发送且已确认ACK     已发送但未收到ACK     未发送但允许发送      未发送且不允许发送
 ┌──────────────────┬──────────────────┬──────────────────┬──────────────────┐
