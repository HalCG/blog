...
```mermaid
flowchart TD
    subgraph MS["渲染阶段：多重采样帧缓冲 (MSFBO)"]
        MS1["颜色附件：多重采样 RBO<br/>（快速写入 / 无采样开销）"]
        MS2["深度附件：多重采样 RBO<br/>（深度测试优化）"]
    end
    subgraph N["解析与采样阶段：常规帧缓冲 (FBO)"]
        N1["颜色附件：普通 2D 纹理"]
        N2["深度附件：普通 RBO（可选）"]
    end
    MS -- "glBlitFramebuffer<br/>（Resolve 解析）" --> N
    N -- "绑定到纹理单元" --> P["片段着色器后处理<br/>sampler2D 读取该纹理"]
    P --> Q["Bloom、灰度化、模糊等操作"]
```
