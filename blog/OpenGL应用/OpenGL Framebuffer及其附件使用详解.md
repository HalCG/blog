...
```mermaid
flowchart TD
    subgraph FBO["帧缓冲对象 (FBO) 结构"]
        C0["颜色挂载点 0<br/>(GL_COLOR_ATTACHMENT0)"]
        D0["深度挂载点<br/>(GL_DEPTH_ATTACHMENT)"]
        S0["模板挂载点<br/>(GL_STENCIL_ATTACHMENT)"]
    end
    C0 -- "关联" --> T["纹理附件<br/>（可读写纹理）"]
    D0 -- "关联" --> R1["渲染缓冲 (RBO)<br/>（只写深度缓冲）"]
    S0 -- "关联" --> R2["渲染缓冲 (RBO)<br/>（只写模板缓冲）"]
```
...
```mermaid
flowchart TD
    A["第一步：渲染到离屏 FBO"] --> B["写入纹理附件"]
    B --> C["场景图元信息暂存"]
    C --> D["FS 采样该离屏纹理<br/>（在全屏四边形上）"]
    D --> E["进行像素处理"]
    E --> F["第二步：绑定默认屏幕 FBO<br/>输出到屏幕"]
```
...
flowchart TD
    FS["片元着色器 (FS)"]
    FS -- "layout(location=0)" --> A0["颜色附件 0<br/>（例如：正常颜色）"]
    FS -- "layout(location=1)" --> A1["颜色附件 1<br/>（例如：材质法线或高光）"]
```
