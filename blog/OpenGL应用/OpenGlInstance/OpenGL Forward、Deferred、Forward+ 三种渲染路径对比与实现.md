...
```
RenderingPathsApp（应用总控）
├── Scene                    测试场景（12个Spot模型实例 + 地板）
├── LightManager             点光源管理器（统一生成光源，CPU Tile 裁剪）
├── ForwardRenderer          前向渲染处理器
├── DeferredRenderer         延迟渲染处理器（管理 G-Buffer FBO 与 Fullscreen Quad）
├── ForwardPlusRenderer      Forward+ 渲染处理器
├── VtkTrackballCamera       相机控制器
└── PerfStats                分 Pass 的 GPU 与 CPU 时间计时器
```
