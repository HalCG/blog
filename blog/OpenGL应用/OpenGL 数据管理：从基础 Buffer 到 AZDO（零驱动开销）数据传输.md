...
```mermaid
flowchart TD
    subgraph IL["1. 交错式 (Interleaved)：对齐紧密，硬件缓存友好（首选）"]
        direction LR
        A1["Pos0"] --- A2["Norm0"] --- A3["UV0"] --- A4["Pos1"] --- A5["Norm1"] --- A6["UV1"] --- A7["..."]
    end
    subgraph BT["2. 批量式 (Batched)：按属性成组分段存储"]
        direction LR
        B1["Positions<br/>(P0, P1, ...)"] --- B2["Normals<br/>(N0, N1, ...)"] --- B3["TexCoords<br/>(U0, U1, ...)"]
    end
```
