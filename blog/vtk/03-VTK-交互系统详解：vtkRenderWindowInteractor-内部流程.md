...
```
┌─────────────────────────────────────────────────────────────┐
│  平台层 (Concrete Subclass)                                  │
│  vtkWin32RenderWindowInteractor / vtkXRenderWindowInteractor  │
│  vtkCocoaRenderWindowInteractor / vtkQRenderWindowInteractor │
│  → 监听 Win32 / X11 / Cocoa / Qt 原生事件                     │
└─────────────────────────────────────────────────────────────┘
                              ▲ 继承
┌─────────────────────────────────────────────────────────────┐
│  抽象层 vtkRenderWindowInteractor                            │
│  → 平台无关的事件循环、Timer、Picking、事件队列               │
└─────────────────────────────────────────────────────────────┘
                              ▲ 持有（非继承）
┌─────────────────────────────────────────────────────────────┐
│  策略层 vtkInteractorStyle 及其子类                           │
│  TrackballCamera / TrackballActor / Image / Joystick / …    │
│  → 把 VTK 事件翻译成旋转、平移、缩放、拾取等业务动作           │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│  渲染层 vtkRenderWindow / vtkRenderer / vtkActor             │
│  → Render() → OpenGL 后端 → SwapBuffers                      │
└─────────────────────────────────────────────────────────────┘
...
```
Win32 消息 → vtkWndProc
  → vtkWin32RenderWindowInteractor::OnMessage()
    → WM_LBUTTONDOWN  → LeftButtonPressEvent
    → WM_MOUSEMOVE    → MouseMoveEvent
    → WM_MOUSEWHEEL   → MouseWheelForwardEvent / BackwardEvent
    → WM_KEYDOWN      → KeyPressEvent
    → WM_SIZE         → ConfigureEvent
    → InvokeEvent(vtkEvent, &eventData)   ★ 进入观察者链
...
```
[OS 原生事件 WM_LBUTTONDOWN]
         │
         ▼
[vtkWin32RenderWindowInteractor::OnMessage]   ← 平台子类
   └─ InvokeEvent(LeftButtonPressEvent, &pos)
         │
         ▼
[vtkObject::InvokeEvent 遍历 Observer 链]
         │
...
[vtkInteractorStyle::OnMouseMove]
   ├─ State == ROTATE → camera->Azimuth/Elevation
   └─ Interactor->Render()
         │
         ▼
[vtkOpenGLRenderWindow::Render]
   ├─ glClear / glViewport
   ├─ RenderEvent → 各 Renderer 绘制 Actor
   └─ SwapBuffers
