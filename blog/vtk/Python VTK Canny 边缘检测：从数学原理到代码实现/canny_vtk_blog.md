...
1. **低错误率**：尽量检测真实边缘，抑制噪声伪边缘；
2. **高定位精度**：检测到的边缘应尽可能贴近真实边缘位置；
3. **单一响应**：同一条真实边缘只产生一次响应，避免“一条边多条线”。
...
其中 $\sigma$ 控制平滑强度：$\sigma$ 越大，噪声抑制越强，但边缘也会被抹糊。
...
经典 Canny 论文使用 Sobel 类算子估计 $G_x, G_y$。VTK 中 `vtkImageGradient` 基于中心差分计算各方向偏导，再配合 `vtkImageMagnitude` 求模。
...
对像素 $(i,j)$，设梯度方向 $\theta$ 落在某一扇区（通常将 $[0,\pi)$ 划分为 4 个方向：$0°$、$45°$、$90°$、$135°$），比较 $(i,j)$ 与其梯度方向上**前后两个邻域像素**的幅值：
...
仅用一个阈值很难区分“强边缘”和“噪声”。Canny 引入**高阈值 $T_h$** 与**低阈值 $T_l$**（通常 $T_l \approx T_h/2$）：

| 像素类型 | 条件 | 含义 |
| :--- | :--- | :--- |
| 强边缘 | $M \ge T_h$ | 肯定是边缘 |
| 弱边缘 | $T_l \le M < T_h$ | 可能是边缘，需跟踪确认 |
| 非边缘 | $M < T_l$ | 直接丢弃 |
...
| :---: | :--- | :--- | :---: |
| 低阈值 $T_l$ | `vtkLinkEdgels` | `SetGradientThreshold()` | 2 |
| 高阈值 $T_h$ | `vtkThreshold` | `SetUpperThreshold()` + `THRESHOLD_UPPER` | 10 |
...
| 参数 | VTK 设置 | 调大效果 | 调小效果 |
| :--- | :--- | :--- | :--- |
| $\sigma$ | `SetStandardDeviations` | 更平滑、更抗噪、边缘更少 | 保留更多细节、噪声增多 |
| 低阈值 $T_l$ | `SetGradientThreshold` | 弱边缘更难被链接 | 更多弱边缘被保留，噪声增加 |
| 高阈值 $T_h$ | `SetUpperThreshold` | 只保留最强边缘 | 更多边缘通过，可能引入伪边缘 |

**经验法则**：

1. 先固定 $\sigma$，观察梯度幅值分布；
2. 设 $T_h$ 为幅值直方图较高分位（如 70%～90%）；
3. 设 $T_l \approx (0.4 \sim 0.5)\,T_h$；
4. 若边缘断裂，降低 $T_l$ 或略降 $T_h$；若噪声多，增大 $\sigma$ 或提高 $T_h$。
...
不是。它们针对具体图像的梯度幅值尺度。不同 bit 深度、是否归一化、$\sigma$ 大小都会影响幅值范围，必须结合可视化迭代调整。
...
1. 图像域：`GaussianSmooth` → `Gradient` → `Magnitude` → `NonMaximumSuppression`
2. 几何域：`ImageToStructuredPoints` → `LinkEdgels` → `Threshold` → `SubPixelPositionEdgels` → `Stripper`
...
2. VTK 官方示例：[CannyEdgeDetector](https://examples.vtk.org/site/PythonicAPI/Images/CannyEdgeDetector/)
3. VTK 源码测试：`Filters/General/Testing/Python/Canny.py`
4. 《VTK 图形图像开发进阶》第 7.4.2 节 Canny 算子
