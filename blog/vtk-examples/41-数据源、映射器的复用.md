...
int main(int, char*[])
{
  ...
  // In this example we create eight different spheres (two rows of four spheres)
  // and set the ambient lighting coefficients.
  ...
  // and then, in a loop, create the object using spheres.push_back(vtkSmartPointer<vtkActor>::New());
  //
  // The reason:
  ...
  // arrays so its elements do not need to be CopyAssignable and CopyConstructible.
  ...
  // Parallel camera projection will be used.
