---
title: VTK 策略模式详解：InteractorStyle、Mapper、Picker 与可替换算法
description: >
  系列入门篇：[05 策略模式](05-strategy.md) · [Qt / VTK 设计模式](../README.md)  本文基于 VTK 官方 API 与 Kitware 开源源码梳理，不依赖任何特定业务项目。  主要参考：`vtkRenderWindowInteractor`、`
---
...
- 为什么 `SetInteractorStyle` 是教科书级策略？
- 换 Mapper 和换 Style 有什么相同与不同？
...
```mermaid
flowchart TD
    CTX["Context<br/>持有 Strategy*<br/>把请求委托给它"] -- uses --> ST["Strategy<br/>抽象接口"]
    ST --> A["ConcreteA"]
    ST --> B["ConcreteB"]
    ST --> C["ConcreteC"]
```

> ConcreteA / B / C 为可互换的具体算法。
...
```mermaid
flowchart TD
    OS["OS 输入<br/>（鼠标/键盘/触摸）"] --> IREN["vtkRenderWindowInteractor<br/>Context（壳，路由事件）"]
    IREN -- "GetInteractorStyle()" --> STYLE["vtkInteractorStyle<br/>Strategy：手势算法<br/>（旋转/框选/窗宽窗位）"]
    STYLE -- "更新 Camera /<br/>Actor / Selection" --> REN["vtkRenderer::Render()"]
    REN --> PROP["vtkActor / vtkVolume"]
    PROP -- "GetMapper()" --> MAP["vtkMapper<br/>Strategy：如何把数据画出来"]
    MAP --> GPU["GPU / 帧缓冲"]
```
...
```mermaid
classDiagram
    vtkObject <|-- vtkInteractorObserver
    ...
    vtkInteractorStyle <|-- vtkInteractorStyleJoystickActor
    vtkInteractorStyle <|-- vtkInteractorStyleRubberBand2D
    vtkInteractorStyle <|-- vtkInteractorStyleRubberBand3D
    vtkInteractorStyle <|-- vtkInteractorStyleRubberBandPick
    vtkInteractorStyle <|-- vtkInteractorStyleImage
    vtkInteractorStyle <|-- vtkInteractorStyleTerrain
    vtkInteractorStyle <|-- vtkInteractorStyleFlight
    vtkInteractorStyle <|-- vtkInteractorStyleMultiTouchCamera
    vtkInteractorStyle <|-- vtkInteractorStyleSwitch
...
```mermaid
flowchart TD
    A["平台：鼠标左键按下"] --> B["vtkRenderWindowInteractor::<br/>LeftButtonPressEvent()"]
    B --> C["vtkInteractorStyle::<br/>OnLeftButtonDown()<br/>（虚函数，子类实现）"]
    C --> D["Trackball：旋转相机<br/>RubberBand：记录起点"]
    D --> E["InvokeEvent(LeftButtonPressEvent)<br/>仍可被外部 observer 监听"]
    E --> F["可能 RenderWindow->Render()"]
```
...
- **复用** `vtkSmartPointer` 保存策略实例，避免每次 `new`
- 切换前可 `GetInteractorStyle()` 存为「上一模式」，便于工具栏退出时恢复
- `SetInteractorStyle` 后检查 `CurrentRenderer` 是否仍正确（多视口时见 §7）
...
- **继承最接近的现有类**（多数情况选 `vtkInteractorStyleTrackballCamera`）
- 只重写需要的 `OnLeftButtonDown` / `OnMouseMove` / `OnChar` 等
- 需要额外逻辑时 `InvokeEvent`，不要绕过基类除非有意为之
- 用 `vtkSetMacro` / 成员保存工具状态（橡皮筋起点、测量点等）
- 在析构或工具退出时 `RemoveObservers`，避免悬空 `clientData`
...
```mermaid
flowchart TD
    A["vtkAlgorithm (Filter)"] --> B["vtkDataSet"]
    B --> C["vtkMapper<br/>Strategy：着色、图元类型、<br/>GPU 路径"]
    C --> D["vtkActor / vtkVolume"]
    D --> E["vtkRenderer"]
```
...
```mermaid
classDiagram
    vtkAbstractPicker <|-- vtkPicker
    vtkAbstractPicker <|-- vtkCellPicker
    vtkAbstractPicker <|-- vtkPointPicker
    vtkAbstractPicker <|-- vtkPropPicker
    vtkAbstractPicker <|-- vtkWorldPointPicker
    vtkAbstractPicker <|-- vtkAreaPicker
    class vtkPicker {
        基于图形硬件遍历 Prop
        常用
    }
...
```mermaid
flowchart LR
    subgraph RW["vtkRenderWindow"]
        R0["Renderer0"]
        R1["Renderer1"]
    end
    I0["Interactor0 + Style0"] <--> R0
    I1["Interactor1 + Style1<br/>（或共享一个 Interactor）"] <--> R1
```
...
- `iren` 自始至终是 Context
- `navStyle` / `selectStyle` 是长期复用的策略实例
- 框选结果通过 **Observer** 传出，不把业务逻辑写进 Style 源码
- 恢复浏览 = 再次 `SetInteractorStyle(navStyle)` —— 纯策略切换
...
- **依赖抽象类型**：函数参数写 `vtkInteractorStyle*` / `vtkMapper*`，不要写死具体子类
- **策略实例复用**：热切换用 `vtkSmartPointer` 缓存，避免每帧 `New()`
- **保存默认 Style**：进入工具前 `auto* prev = iren->GetInteractorStyle()`，退出时恢复
- **自定义 Style 继承最近子类**：少重写、多 `Superclass::OnLeftButtonDown()`
- **Mapper 与数据类型对照表** 写进项目文档，防止接错
- **拾取容差** `SetTolerance` 按场景调，单元过密时 `CellPicker` 可能变慢
- **多视口** 换 Style 时检查 `CurrentRenderer`
...
```mermaid
flowchart TD
    IREN["vtkRenderWindowInteractor<br/>SetInteractorStyle(...)"]
    IREN --> S1["TrackballCamera<br/>（浏览策略）"]
    IREN --> S2["RubberBand2D<br/>（框选策略）"]
    IREN --> S3["ImageStyle<br/>（窗宽窗位策略）"]
    S1 --> REN["vtkRenderer::Render()"]
    S2 --> REN
    S3 --> REN
    REN --> ACT["vtkActor<br/>SetMapper(...)"]
    REN --> VOL["vtkVolume<br/>SetMapper(...)"]
    ACT --> M1["PolyDataMapper<br/>（面片策略）"]
    VOL --> M2["GPUVolumeRayCastMapper<br/>（体绘制策略）"]
    M1 --> FB["帧缓冲 / 显示"]
    M2 --> FB
```
