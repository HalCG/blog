...
多重采样操作 (Multisample)
       ↓
模板测试 (Stencil Test)         ← (必须在传统深度测试之前)
       ↓
传统深度测试 (Depth Test)        ← (常规执行阶段)
       ↓
混合 (Blending)
       ↓
抖动与逻辑操作 (Dithering & Logical Ops)
       ↓
