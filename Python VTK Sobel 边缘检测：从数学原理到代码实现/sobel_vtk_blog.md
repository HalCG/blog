...
2. Sobel 核的外积推导
3. 3×3 卷积的逐像素展开
4. Python VTK 中的 `vtkImageSobel2D` 与 `vtkImageConvolve` 实现
...
设连续图像函数为 $f(x, y)$，其一阶偏导数描述灰度沿 $x$、$y$ 方向的变化率：
...
在一维中，$f'(x) = df/dx$ 表示函数变化快慢。二维图像中，边缘通常对应 $|\nabla f|$ 的局部极大值。因此，任何实用的边缘算子，最终目标都是估计 $f_x$、$f_y$，再合成梯度幅值。

---

## 3. 离散差分：像素网格上的导数近似

数字图像是离散采样，不能直接求 $\partial f/\partial x$，只能用**差分**近似。
...
$D_x$、$D_y$ 只对 3 个像素做运算，**对噪声极其敏感**：单个噪点即可产生虚假“边缘”。Sobel 的改进思路是：**在求导方向保持差分结构，在垂直方向引入平滑**。
...
对 **x 方向梯度** $G_x$（检测**垂直边缘**，即灰度沿 x 变化）：
...
对 **y 方向梯度** $G_y$（检测**水平边缘**）则对称：y 方向差分，x 方向平滑。
...
平滑核 $[1, 2, 1]$ 可看作小尺度高斯平滑的离散近似，中心像素权重为 2，强调当前行（或列）的信息。
...
- 第 1 行：$1 \times [-1,0,1] = [-1,0,1]$
- 第 2 行：$2 \times [-1,0,1] = [-2,0,2]$
- 第 3 行：$1 \times [-1,0,1] = [-1,0,1]$
...
$$
S_x = \begin{bmatrix} 1 & 2 & 1 \end{bmatrix}, \quad
D_y = \begin{bmatrix} -1 \\ 0 \\ 1 \end{bmatrix}
$$
...
即 $G_y = G_x^\top$（在标准 Sobel 定义下）。

### 4.5 示意图

![Sobel 算子外积分解：差分核与平滑核相乘得到 3×3 卷积核](sobel_kernels.svg)
...
设 $f_{i,j}$ 表示行 $i$、列 $j$ 的灰度（$i$ 向下增大，$j$ 向右增大），以 $(i,j)$ 为中心的 3×3 邻域为：
...
$$
\begin{aligned}
G_x(i,j) =\;& (-1)\,f_{i-1,j-1} + 0\cdot f_{i-1,j} + 1\cdot f_{i-1,j+1} \\
           &+ (-2)\,f_{i,j-1}   + 0\cdot f_{i,j}   + 2\cdot f_{i,j+1}   \\
           &+ (-1)\,f_{i+1,j-1} + 0\cdot f_{i+1,j} + 1\cdot f_{i+1,j+1}
\end{aligned}
$$
...
$$
\begin{aligned}
G_y(i,j) =\;& -f_{i-1,j-1} - 2f_{i-1,j} - f_{i-1,j+1} \\
...
1. **求导方向必须用差分核**，否则无法估计一阶导数。
2. **垂直方向必须用平滑核**，否则噪声会被差分放大。
3. **Sobel = 求导方向差分 × 垂直方向平滑（外积）**。

### 6.1 为何 x 方向不能也平滑？

若在 x 方向也使用 $[1, 2, 1]$ 平滑，会把“求导”变成“先平滑再求导”，导致：
...
| 算子 | 平滑权重 | 特点 |
|------|----------|------|
| Sobel | $[1, 2, 1]$ | 经典，计算快，工程最常用 |
| Prewitt | $[1, 1, 1]$ | 权重均匀，更平滑、边缘更宽 |
...
| `vtkImageExtractComponents` | `vtkImagingCore` | 提取 $G_x$ / $G_y$ 分量 |
| `vtkImageMathematics` | `vtkImagingMath` | 计算 $\sqrt{G_x^2 + G_y^2}$ |
...
```
输入图像 → vtkImageSobel2D → 2D 向量场 (Gx, Gy)
                              ├─ component 0 → Gx
                              └─ component 1 → Gy
Gx, Gy → 平方 → 相加 → 开方 → |∇f|
```
...
1. 用 Matplotlib 绘制 Sobel 外积分解图 → `sobel_kernels.png`（同目录已附带 `sobel_kernels.svg` 示意图）
2. 用 VTK 在合成图像上运行 Sobel → `sobel_vtk_result.png`（同目录已附带 `sobel_vtk_result.svg` 示意图）
...
![VTK Sobel 边缘检测：原图、Gx、Gy、梯度幅值及手写核验证](sobel_vtk_result.svg)
...
- 显示为灰度图前，通常取绝对值并做 `vtkImageShiftScale` 归一化到 $[0, 255]$
...
| Sobel | $G_x = S_y \otimes D_x$，求导方向差分、垂直方向平滑 |
...
1. 一阶导数 / 梯度是边缘检测的理论基础。
2. 离散差分对噪声敏感；Sobel 通过外积引入平滑，更稳定。
3. Python VTK 中，`vtkImageSobel2D` + `vtkImageMathematics` 即可完整实现 Sobel 梯度与边缘强度计算。
